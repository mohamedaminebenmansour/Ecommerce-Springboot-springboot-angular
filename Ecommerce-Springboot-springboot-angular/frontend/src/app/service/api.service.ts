...
export class ApiService {
  ...
  addProduct(formData: any): Observable<any> {
    console.log('formData:', formData);  // Log the form data before sending the request
    return this.http.post(`${ApiService.BASE_URL}/product/create`, formData, {
      headers: this.getHeader()
    }).pipe(
      tap(response => console.log('API response:', response)),  // Log the API response on success
      catchError(error => {
        console.error('API error:', error);  // Log the error object on failure
